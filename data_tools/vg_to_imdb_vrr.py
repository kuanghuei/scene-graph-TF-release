...
def add_images(im_data, vrrvg_dir, h5_file, args):
    fns = []; ids = []; idx = []
    ...
    for i, img in enumerate(im_data):
        ...
        xml_filename = os.path.join(args.vrrvg_dir, str(img['image_id']) + '.xml')
        if os.path.exists(filename) and os.path.exists(xml_filename):
            fns.append(filename)
...
def main(args):
    ...
    # load images
    im_fns = add_images(im_metadata, args.vrrvg_dir, f, args)
